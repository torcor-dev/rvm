...
from rvm.db.models import Media, RedditMeta, Subreddit, Tag, Video
...
class Manager:
    ...
    def add_query_filter(self, filter, condition):
        ...
        if filter == Filters.MIN_DURATION:
            self.query.add_outer_join(Video)
            self.query.add_filter(
                (Video.duration.cast(Float) > condition)
                | ((Media.format == "gif") & (Media.size > (condition * 1_000_000)))
            )

    def _exclude_subs(self, condition):
        alias_sub = aliased(Subreddit)
        subquery = (
            self.db.session.query(alias_sub.name)
...
class TagManager:
    ...
    def tag_subs(self, subs, tags):
        ...
        for sub in subs:
            sub = (
                self.db.session.query(Subreddit)
    ...
    def untagged_subs(self):
        ...
        subquery = (
            self.db.session.query(Subreddit.name)
            .join(Subreddit.tags)
            .group_by(Subreddit.name)
        )
        query = self.db.session.query(Subreddit.name).filter(
            Subreddit.name.not_in(subquery)
        )
